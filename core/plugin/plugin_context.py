...
from ..provider import ProviderManager, LLMModelClient, EmbeddingModelClient, ModelGroupManager, ModelGroup
...
if TYPE_CHECKING:
    ...
    from core.prompt_block import PromptBlockRegistry
    from core.pipeline.hints_pipeline import HintsPipeline
...
class PluginContext:
    ...
    # Phase 0.1: model group middleware. Plugins should prefer get_model_group()
    # over directly resolving providers, so future cooldown/fallback logic works
    # transparently. The legacy provider_mgr access remains valid for now.
    ...
    # Phase 0.2 M3: PromptBlock registry. Plugins read this when they need
    # to introspect what other plugins have contributed (rare); the typical
    # registration flow is the @register.prompt_block decorator + lifecycle
    # wiring inside PluginManager._register_plugin_prompt_blocks_for, not
    # direct calls into this attribute. Marked Optional because old test
    # harnesses may construct PluginContext without it.
    prompt_block_registry: Optional["PromptBlockRegistry"] = None

    # Phase 0.4 M5: hints async pipeline. Plugins typically register
    # handlers via @register.hints_handler decorators (bound during
    # PluginManager.init_plugin); imperative ctx.hints_pipeline.register
    # is also supported for plugins that decide their handler set at
    # runtime. Optional for the same reason as prompt_block_registry —
    # PluginContext may be constructed in degraded test harnesses
    # without an HintsPipeline being wired in.
    hints_pipeline: Optional["HintsPipeline"] = None
    ...
    def get_model_group(self, group_id: str) -> Optional[ModelGroup]:
        """
        Phase 0.1: resolve a ModelGroup by id. Use the virtual ids
        '__default__' / '__default_fast__' to address the single-model groups
        derived from `models.default_llm` / `models.default_fast_llm`. User
        groups defined in data/config/model_groups.json are addressed by their
        own group_id.
        """
        if self.model_group_mgr is None:
            return None
        return self.model_group_mgr.get_group(group_id)
