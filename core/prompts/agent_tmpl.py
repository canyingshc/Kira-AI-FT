# DEPRECATED(Phase 0 → Phase 1): 整个模块未来由 M4 Translator + Jinja2 模板接管。
# Phase 0.2 M3 起，PromptManager.get_agent_prompt() 把这里的 11 个模板包成
# PromptBlock 经 PromptAssembler 求值，但模板字符串本身仍保留在此处。
# M4 落地时迁移到 core/prompts/translator/*.j2，本模块作为兼容 import 保留；
# Phase 1 完成所有插件迁移后整体删除。
