"""OpenAI provider client (Phase 0 M7-merge with "旧文件" v1/chat 生图).

Layered changes vs current core/provider/src/openai/model_clients.py:

1. ``OpenAILLMClient.chat`` — UNCHANGED. Critically, exceptions still
   re-raise (NOT wrapped in ``LLMResponse(text_response="[Error]...")``)
   so that :class:`~core.provider.model_group.ModelGroup` can classify
   them via ``_classify_error`` and apply 429/5xx/timeout cooldown.
   The "旧文件" version swallowed and stringified them — taking that
   would silently disable Phase 0.1 M2 cooldown logic.

2. ``OpenAIImageClient`` — MERGED with the dual-mode v1/chat 生图 from
   the user's previous-version invasive modification. New private
   :data:`USE_CHAT_COMPLETIONS` class flag picks between:
     False → ``/v1/images/generations`` (original behaviour)
     True  → ``/v1/chat/completions``    (multimodal chat models that
                                          return images via Markdown
                                          ``![alt](url)`` or data URIs
                                          or output_image content parts)
   Plus the helper trio ``_extract_image_from_message`` /
   ``_scan_parts`` / ``_try_extract_image_part`` that walks SDK +
   third-party-compatible response shapes safely (never extracting
   bare URLs from text content).

3. ``OpenAIEmbeddingClient`` — UNCHANGED. The "旧文件" version was
   identical apart from the now-stale commented-out alternative
   implementation, which we preserve for git-history continuity.

The ``USE_CHAT_COMPLETIONS`` switch is intentionally a class attribute
rather than per-instance config. Phase 0 doesn't introduce a new
config namespace for image client modes — flipping it remains a code
change. If/when this becomes runtime-configurable, route it through
``data/config/providers/openai.json`` like other provider settings,
NOT through bot_config.
"""
...
class OpenAILLMClient(LLMModelClient):
    ...
    async def chat(self, request: LLMRequest, **kwargs) -> LLMResponse:
        ...
        try:
            ...
            # Re-raised so ModelGroup._classify_error can see the status
            # code and apply 429 / 5xx cooldown. DO NOT wrap into
            # LLMResponse(text_response="[Error]...") — that pattern from
            # the pre-Phase-0 codebase silently disabled fallback.
            raise
...
class OpenAIImageClient(ImageModelClient):
    ...
    # ────────────────────────────────────────────────────────────
    # Mode toggle (manual switch — Phase 0 does not expose this via
    # config; keep it here as a code-level flag).
    #   False → /v1/images/generations  (legacy)
    #   True  → /v1/chat/completions    (multimodal chat-mode生图)
    # ────────────────────────────────────────────────────────────
    ...
    async def image_to_image(
        ...
        # Not yet implemented in either mode — placeholder kept to
        # satisfy ImageModelClient ABC. Will be wired up alongside the
        # generations endpoint variant when needed.
        pass
