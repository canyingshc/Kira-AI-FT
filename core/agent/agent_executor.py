...
from core.provider import LLMRequest, LLMResponse, LLMModelClient, ModelGroup
...
class AgentExecutionContext:
    ...
    # Phase 0.1 M2: optional ModelGroup. When present, every agent step calls
    # group.call() (which handles cooldown / fallback / validation across the
    # group's entries). When None, the executor falls back to llm_model.chat()
...
class AgentExecutor:
    ...
    async def run(
        ...
        if model_group is not None:
            llm_logger.info(
                f"[{sid}] Running agent via group '{model_group.group_id}' "
                f"(primary: {model_id} / {provider_name})"
            )
        else:
        ...
        for step in range(max_steps):
            ...
            try:
                # M2: prefer the group path so 429/5xx/timeout fallback applies on
                # every step, not just the first. Falls back to direct chat() when
                # no group is wired up (legacy callers).
                if model_group is not None:
                    llm_resp = await model_group.call(request)
                else:
